  ...
  const onToChainPopup = ({ chatUuid, chatValue }) => {
    $post("/sendMsg", { msg: chatValue.value }).then(
      (res) => {
        ...
        visible.value = false;
        loading.value = false;
        message.success("Uplink chat successfully!");
      },
      (err) => {
        setTimeout(() => {
          console.log("err==>", err);
